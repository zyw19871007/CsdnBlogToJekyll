...
def CrawlingItemBlog(base_url, id):
    ...
    if item_html.status_code == 200:    # 200说明request_get完成，这是因为http协议里面定义的状态码
        ...
        # 获取文章发表时间
        v_time = soup.find(attrs={'class': 'time'}).get_text()
        ...
        for img in all_img:     # 能进入该循环则说明该篇博客中有图片
            ...
            if len(img_url)==0:
                img_url = re.findall(r"(?=img-blog)[^\"]*", img_str)
...
if __name__ == "__main__":
    username = 'zyw19871007'        # CSDN 个人主页地址中的用户名，例如https://blog.csdn.net/Tang_Chuanlin中的Tang_Chuanlin
